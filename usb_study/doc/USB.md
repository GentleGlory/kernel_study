...
# OTG 接口電路
- 檢測ID引腳。 Type C 的話 CC1 、 CC2 引腳。如果 ID 引腳為高電平，則 USB 工作於 Device 模式。 如果為低電平，則工作於 Host 模式。

- USB Type C 有比較高成本的做法，就是將 CC1 、 CC2 接入一個專門的 Chip 。這個 Chip 會監測 CC1 和 CC2 的變化。透過中斷通知 SOC ， USB 要工作在哪一種模式。
